...
public class AgendaService implements IAgendaService {
  ...
  public void update(Long id, AgendaRequestPutDto agendaRequestPutDto, BindingResult bindingResult) {
    ...
    if (agendaRequestPutDto.description() != null
        && !agendaRequestPutDto.description().equals(agenda.getDescription())) {
  ...
  public void startVoting(Long id, Integer durationSeconds) {
    ...
    boolean hasOpenAgenda = agendaRepository.existsBySessionIdAndStatusAndIdNot(
        agenda.getSession().getId(), AgendaStatus.OPEN, agenda.getId());

    if (hasOpenAgenda) {
      throw new VotingException("Another agenda in this session is currently open for voting");
    }
  ...
  public void stopVoting(Long id) {
    ...
    if (agenda.getStatus() != AgendaStatus.OPEN) {
      throw new VotingException("Voting is not currently open for this agenda");
    }

    agenda.setStatus(AgendaStatus.CLOSED);
    agenda.setEndVotingAt(LocalDateTime.now());
    agendaRepository.save(agenda);
  }
