...
public interface VoteRepository extends JpaRepository<VoteModel, Long> {
  ...
  @Query("SELECT v FROM VoteModel v WHERE v.associateId.id = :associateId AND v.agenda.id = :agendaId")
  Optional<VoteModel> findByAssociateIdAndAgendaId(@Param("associateId") Long associateId,
